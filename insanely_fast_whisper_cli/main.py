...
def main(model, device, dtype, batch_size, better_transformer, chunk_length, audio_file):
    ...
    with open(srt_filename, 'w') as srt_file:
        ...
        for index, chunk in enumerate(outputs['chunks']):
            prev, start_time = __seconds_to_srt_time_format(prev, chunk['timestamp'][0])
            prev, end_time = __seconds_to_srt_time_format(prev, chunk['timestamp'][1])
            srt_file.write(f"{index + 1}\n")
            srt_file.write(f"{start_time} --> {end_time}\n")
            srt_file.write(f"{chunk['text'].strip()}\n\n")

def __seconds_to_srt_time_format(prev, seconds):
